...
{
    public class UpdateAccessoryIndicativePriceValidator : AbstractValidator<UpdateAccessoryIndicativePriceModel>
    {
        public UpdateAccessoryIndicativePriceValidator()
        {
            RuleFor(x => x.Accessoryindicativepriceid).NotEmpty().WithMessage("Accessory Indicative Price id cannot be null or empty")
                   .Must(BeValidNumber).WithMessage("Accessory Indicative Price id cannot be zero or less than zero");
