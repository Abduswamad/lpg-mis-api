...
{
    public class UpdateCylinderIndicativePriceValidator : AbstractValidator<UpdateCylinderIndicativePriceModel>
    {
        public UpdateCylinderIndicativePriceValidator()
        {
            RuleFor(x => x.Cylinderindicativepriceid).NotEmpty().WithMessage("Cylinder Indicative Price id cannot be null or empty")
                   .Must(BeValidNumber).WithMessage("Cylinder Indicative Price id cannot be zero or less than zero");
